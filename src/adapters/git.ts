...
abstract class GitAdapter implements IRepoAdapter {
  ...
  public async checkoutRepo(repo: IRepo, originBranch?: string): Promise<void> {
    ...
    if (await fs.existsAsync(localPath) && await this.git(repo).checkIsRepo()) {
      ...
      if (originBranch) {
        gitArgs.push('-b', originBranch, '--single-branch');
      }
  ...
  public abstract createPullRequest(repo: IRepo, message: string, targetBranch?: string): Promise<void>;
