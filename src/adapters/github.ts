...
class GithubAdapter extends GitAdapter {
  ...
  // private async getOrgRepos(org: string, onRetry: RetryMethod): Promise<IRepo[]> {
  ...
  // private async getCodeSearchRepos(search_query: string, onRetry: RetryMethod): Promise<IRepo[]> {
  ...
  //     }
  //      return this.getOrgRepos(org, onRetry);
  //   }
  //   // otherwise, github code search query.  results are less reliable
  //   return this.getCodeSearchRepos(search_query, onRetry);
  // }
  public async getCandidateRepos(onRetry: RetryMethod): Promise<IRepo[]> {
    const { org, search_query } = this.migrationContext.migration.spec.adapter;
    let repoNames = [];
